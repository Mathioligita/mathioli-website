...
const CheckoutPage = () => {
  ...
  const [paynowbutton, setPaybutton] = useState(false);
  const [editshippingfromdata, setEditshippingfromdata] = useState(null);
  const [paynowbuttonsuccess, setPaybuttonsuccess] = useState([]);
  const [singleselectbooks, setSingleselectBooks] = useState([]);
  const [SingleBuyProductdata, setSingleBuyProduct] = useState([]);
  const guestId = Cookies.get("guestId");
  const router = useRouter();
  ...
  // const handlePayment = async () => {
  //   try {
  //     console.log("inside the handle payment starting")
  //     console.log("inside the handle payment starting checking the userData",usersdata)
  //     console.log("inside the handle payment starting checking the checkout",checkout)
  //     const activeAddress =
  //       usersdata?.shippingAddress.find((addr) => addr.active) ||
  //       usersdata.shippingAddress[0];
  //       const shippingAddress = {
  ...
  const handlePayment = async () => {
    try {
      console.log("inside the handle payment starting");
      console.log("checking the userData", usersdata);
      console.log("checking the checkout", checkout);
                    ...
                    {activeAddress ? (
                      ...
                      >
                        <input
                          ...
                          className="radio-button me-2"
                          name="address"
                          defaultChecked={activeAddress?._id}
                        />{" "}
                        {activeAddress?.address},{activeAddress?.city},
                        {activeAddress?.state},{activeAddress?.country},
                        {activeAddress?.zipCode}
